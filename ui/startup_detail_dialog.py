...
class StartupDetailDialog(QDialog):
    ...
    def __init__(self, startup_data: dict, parent=None):
        ...
        self.setWindowTitle(
            f"Startup Item Details - {startup_data.get('name', 'Unknown')}"
        )
    ...
    def _add_item_info_section(self):
        ...
        is_enabled = self.startup_data.get("enabled", False)
        status_text = (
            "Enabled (runs at startup)" if is_enabled else "Disabled (will not run)"
        )
        ...
        # Location
        location = self.startup_data.get(
            "location", self.startup_data.get("path", "N/A")
        )
    ...
    def _update_description(self):
        ...
        description = StartupDescriber.get_description(
            check_name, technical=not self.simple_mode
        )
