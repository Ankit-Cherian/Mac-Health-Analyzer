...
class AnimationHelper:
    ...
    @staticmethod
    def fade_in(
        widget: QWidget, duration: int = 300, delay: int = 0
    ) -> QPropertyAnimation:
        """
    ...
    def staggered_fade_in(
        widgets: list, duration: int = 300, delay_increment: int = 50
    ) -> QParallelAnimationGroup:
        """
        ...
        for i, widget in enumerate(widgets):
            animation = AnimationHelper.fade_in(
                widget, duration, delay=i * delay_increment
            )
    ...
    @staticmethod
    def smooth_scroll(
        widget: QWidget, target_value: int, duration: int = 400
    ) -> QPropertyAnimation:
        """
    ...
    @staticmethod
    def fade_transition(
        old_widget: QWidget, new_widget: QWidget, duration: int = 300
    ) -> QSequentialAnimationGroup:
        """
...
def apply_table_row_stagger(
    table_widget, duration: int = 300, delay_increment: int = 30
):
    """
...
def animate_value_change(
    widget: QWidget, property_name: bytes, start_value, end_value, duration: int = 500
) -> QPropertyAnimation:
    """
