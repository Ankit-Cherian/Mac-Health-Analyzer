...
class ProcessesTab(QWidget):
    ...
    def update_metrics(self):
        ...
        self.memory_card.update_value(format_percentage(mem_percent), mem_status)

        # CPU usage
        cpu_percent = summary["cpu_percent"]
        cpu_status = self.get_status_from_percent(cpu_percent)
        self.cpu_card.update_value(format_percentage(cpu_percent), cpu_status)
    ...
    def populate_table(self, processes: list):
        ...
        try:
            ...
            for row, proc in enumerate(processes):
                ...
                # Color code by usage
                if mem_percent > 10:
                    mem_percent_item.setForeground(
                        self.get_color_for_percent(mem_percent)
                    )
                ...
                # Color code by usage
                if cpu_percent > 10:
                    cpu_percent_item.setForeground(
                        self.get_color_for_percent(cpu_percent)
                    )
    ...
    def on_force_quit(self):
        ...
        if not selected_rows:
            QMessageBox.information(
                self, "No Selection", "Please select processes to force quit."
            )
            return
        ...
        # Confirm
        process_list = "\n".join(
            [f"- {name} (PID: {pid})" for name, pid in zip(names, pids)]
        )
    ...
    def on_process_double_clicked(self, row: int, column: int):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to show process details: {str(e)}"
            )
