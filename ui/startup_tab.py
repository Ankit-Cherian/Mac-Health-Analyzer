...
class StartupTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # Hint label
        hint_label = QLabel(
            "💡 Double-click any item to see detailed information and recommendations"
        )
    ...
    def create_table(self) -> QTableWidget:
        ...
        table.setHorizontalHeaderLabels(
            ["Name", "Type", "Status", "Location", "Toggle"]
        )
    ...
    def apply_filters(self):
        ...
        if filter_type == "Login Items":
            filtered_items = [
                item for item in filtered_items if item["type"] == "Login Item"
            ]
        elif filter_type == "Launch Agents":
            filtered_items = [
                item for item in filtered_items if item["type"] == "Launch Agent"
            ]
        elif filter_type == "Launch Daemons":
            filtered_items = [
                item for item in filtered_items if item["type"] == "Launch Daemon"
            ]
        elif filter_type == "Enabled Only":
            filtered_items = [
                item for item in filtered_items if item.get("enabled", True)
            ]
        elif filter_type == "Disabled Only":
            filtered_items = [
                item for item in filtered_items if not item.get("enabled", True)
            ]
    ...
    def on_disable_selected(self):
        ...
        if not selected_rows:
            QMessageBox.information(
                self, "No Selection", "Please select items to disable."
            )
            return
    ...
    def on_item_double_clicked(self, row: int, column: int):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", f"Failed to open item details: {str(e)}")
